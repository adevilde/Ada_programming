...
package body code is
	...
	begin
		...
		while not End_Of_File(File) loop
			...
			Affecter(tab_branche(Integer'Val(byte)), byte, La_freq(tab_branche(Integer'Val(byte))) + 1, neant, neant);
		end loop;
		...
		for i in 1..2**8 loop
			if La_freq(tab_branche(i)) = 0 then
				...
				Affecter(tab_branche(i), Le_byte(tab_branche(i)), file_size + 1, neant, neant);
			end if;
		end loop;
	...
	-- Construire la table de codage de Huffman suivant un parcours infixe de l'arbre
