...
procedure test_code is
	...
	package code_0 is new code_preclean(file_size => length, nbu_byte => 0);
	
	package code_length is new code_preclean(file_size => length, nbu_byte => code_0.nb_caractere_unique("exemple_huff.txt"));
	use code_length;
...
begin
	...
	for i in 1..255 loop
		Acces_tab_code(tab_code, i, val, len);
		if len /= 0 then New_line; c := c+1 ;put(character'val(i-1)); 
		put(len);
		for j in 1..len loop
			put(Acces_code(val, j));
		end loop;
		end if;
	end loop;
	New_line;
